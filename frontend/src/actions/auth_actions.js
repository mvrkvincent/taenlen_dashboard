...
export const fetchUser = () => async (dispatch, getState) => {
  ...
  try {
    const res = await axios.get('http://taenlen.herokuapp.com/api/auth/user', tokenConfig(getState));
...
export const login = user => async dispatch => {
  ...
  try {
    const res = await axios.post('http://taenlen.herokuapp.com/api/auth/login', user, config);
...
export const logout = () => async (dispatch, getState) => {
  await axios.post('http://taenlen.herokuapp.com/api/auth/logout', null, tokenConfig(getState));
  dispatch({
    type: LOGOUT_SUCCESS
  });
};
